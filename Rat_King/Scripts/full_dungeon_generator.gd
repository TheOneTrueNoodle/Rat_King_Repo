...
func _physics_process(delta):
	...
	for i in range(newRooms.size()-1, -1, -1):
		...
		#Else, add that shit to the FUCKING DUNGEON BABY
			...
			for j in newRooms[i].connected_rooms.keys():
				if newRooms[i].connected_rooms[j] != null:
					hasConnection = true
					break
	...
	if currentSize > 0:
		for i in dungeon.keys():
			...
			if(randi_range(0,100) < generation_chance) and currentSize > 0:
				...
				#East
				if direction < 1 and dungeon[i].eastDoor != null and dungeon[i].connected_rooms["East"] == null:
				...
				#West
				elif direction < 2 and dungeon[i].westDoor != null and dungeon[i].connected_rooms["West"] == null:
				...
				#North
				elif direction < 3 and dungeon[i].northDoor != null and dungeon[i].connected_rooms["North"] == null:
				...
				#South
				elif direction < 4 and dungeon[i].southDoor != null and dungeon[i].connected_rooms["South"] == null:
